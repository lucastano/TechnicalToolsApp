...
import { logout, selectEmpresa } from '../store/auth/authSlice';

 let pages = ['Reparaciones', 'Clientes', 'Tecnicos','Reportes','Empresa'];
  ...
  <Container maxWidth="xl">
    <Toolbar disableGutters>
      ...
      <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'none' } }}>
        ...
        >
          {pages.map((page) => (
            <MenuItem   key={page} onClick={() => tabOptions(page)}>
      ...
      <Box sx={{ flexGrow: 1, display: { xs: 'none', md: 'flex' } }}>
        {pages.map((page) => (
          <Button
            ...
            sx={{  color: 'white', display: 'block', fontSize:13 }}
          >
            {page}
          </Button>
        ))}
      ...
      <Box sx={{ flexGrow: 0, display: 'flex', alignItems: 'center', gap: 1 }}>
        ...
        <Tooltip title="Open settings">
          <IconButton onClick={handleOpenUserMenu} sx={{ p: 0 }}>
            <Avatar alt="Remy Sharp" src={empresa.foto} />
          </IconButton>
        </Tooltip>
        <Menu
          sx={{ mt: '45px' }}
        ...
        >
          {settings.map((setting) => (
            <MenuItem key={setting} onClick={() => menuOptions(setting)}>
